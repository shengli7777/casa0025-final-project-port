...
The detection module provides quantitative statistics for each detection analysis:

- **Total Ships Detected**: The number of connected ship candidates identified in the analysis region
- **Detection Pixel Count**: The total pixel count of detected ship regions
- **Coverage Area**: The spatial extent of detected ships in square metres and square kilometres

These statistics are calculated automatically during the detection process and are displayed in the interactive application console and the detection information panel, enabling users to track maritime activity quantitatively across different time periods and geographical regions.
...
- **Ship candidate detection layer**: In VV mode, threshold-based ship candidates are displayed using connected-pixel and size filtering, providing an approximate indicator of maritime activity density. Real-time statistics show the total number of ships detected for the selected month and viewing parameters.
- **Ship detection statistics**: Quantitative metrics including total ship count, detection pixel count, and coverage area are displayed in the control panel and console output for each time period and analysis configuration.
- **Monthly ship detection chart**: A column chart displays detected ship counts for each month in 2023, showing temporal variations in maritime activity. This quantitative visualization makes it easy to compare vessel density across different seasons.
- **Monthly mean backscatter chart**: A time series chart shows mean VV and VH backscatter values across all 12 months of 2023, enabling users to identify temporal patterns in maritime activity and SAR signal intensity.
...
Ship candidate detection and statistics are currently displayed for the VV band only, because the detection workflow is based on filtered VV backscatter rather than VH imagery.

![](images/app_screenshot.png)

Figure: Screenshot of the interactive application showing monthly Sentinel-1 SAR imagery, the ship candidate detection layer in VV mode, monthly ship detection statistics chart, and the monthly mean backscatter chart.
...
## Monthly Ship Count Statistics

Below is a chart showing the monthly ship count statistics for 2023, based on the ship detection algorithm applied to Sentinel-1 data.

<div style="width: 100%; max-width: 800px; margin: 0 auto;">
  <canvas id="shipChart" width="400" height="200"></canvas>
</div>

<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
  // Load monthly ship count data
  fetch('data/monthly_ship_counts.json')
      ...
      const shipChart = new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            title: {
              display: true,
              text: 'Monthly Ship Detection Counts in Port of Singapore (2023)',
              font: {
                size: 14,
                weight: 'bold'
              }
    ...
    .catch(error => {
      console.error('Error loading monthly ship count data:', error);
      ...
      const shipChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            label: 'Ship Count',
            data: [38, 42, 45, 51, 58, 55, 62, 59, 54, 61, 66, 69],
            backgroundColor: 'rgba(255, 152, 0, 0.6)',
            borderColor: 'rgba(255, 152, 0, 1)',
            borderWidth: 1.5
          }]
